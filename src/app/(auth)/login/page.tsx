...
import { useSearchParams, useRouter } from 'next/navigation'; // 💡 FIXED: Added useRouter here
...
export default function AuthPage() {
  ...
  useEffect(() => {
    ...
    // Resets token verification step if switching tabs externally
    setStep(1); 
  }, [mode]);

  // Step 1: Handle sending the OTP code to the user's email
  const handleRequestOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'A luxury validation code has been sent to your inbox.' });
      setStep(2); // Advance to the OTP code verification screen
    } catch (err: any) {
  ...
  // Step 2: Handle verifying the 6-digit OTP code
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || 'Invalid or expired verification code.');

      // Redirect user upon successful handshake (e.g., to account dashboard or shop checkout)
  ...
  // Trigger social provider OAuth redirect flows
  ...
  return (
    <div className={styles.pageWrapper}>
      {/* Editorial Close Button - now tracks browser history back-step */}
      <button 
        onClick={() => router.back()} 
      ...
      <div className={styles.authCard}>
        {/* Dynamic Editorial Typography Architecture */}
        <header className={styles.authHeader}>
          <h1 className={styles.authTitle}>
            {step === 1 ? (isRegister ? 'Create an Account' : 'Welcome back!') : 'Verify Identity'}
          </h1>
          <p className={styles.authSubtitle}>
            {step === 1 
              ? (isRegister ? 'Register to join the atelier.' : 'Log in to access your account.') 
              : `Enter the 6-digit secure key sent to ${email}`
            }
          </p>
        </header>
        ...
        {/* STEP 1: INITIAL COMPILATION (Email / Name collection) */}
        {step === 1 && (
        ...
        {/* STEP 2: VERIFICATION LOCKPAD (6-Digit input field) */}
        {step === 2 && (
          <form onSubmit={handleVerifyOTP} className={styles.authForm}>
            <div className={styles.formGroup}>
              <label className={styles.inputLabel} htmlFor="otp">Verification Code *</label>
              <input
                id="otp"
                type="text"
                required
                ...
                value={otp}
                onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))} // Restricts strictly to digits
              />
            </div>
        ...
        {/* CORE LUXURY OAUTH NODES (Only rendered on Step 1) */}
        {step === 1 && (
          <>
            ...
            <div className={styles.socialRow}>
              ...
              >
                {/* Apple Monochromatic Vector Asset */}
